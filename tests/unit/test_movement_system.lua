...
function TestMovementSystem.testCreateMovement()
    ...
    assert(unit.tu >= 0, "Unit should have current TU")
    assert(unit.maxTu > 0, "Unit should have max TU")
...
function TestMovementSystem.testInsufficientTU()
    ...
    assert(moveCost > unit.tu, "Movement should cost more TU than available")

    -- Movement should fail
    local canMove = unit.tu >= moveCost
...
function TestMovementSystem.testMovementRange()
    ...
    local tuPerTile = 4
    local maxRange = math.floor(unit.tu / tuPerTile)
...
function TestMovementSystem.testMovementCancellation()
    ...
    assert(unit.tu == initialTU, "Cancelled movement should not consume TU")
end
...
function TestMovementSystem.testMovementCompletion()
    ...
    -- Simulate completion
    unit.tu = initialTU - moveAction.tuCost
    unit.position = targetPos

    assert(unit.tu < initialTU, "TU should be consumed")
