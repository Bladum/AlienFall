---Tactical Pathfinding System for Hex Grids
---
---Implements A* pathfinding algorithm optimized for hexagonal tactical combat grids.
---Supports terrain movement costs, Time Unit (TU) calculation, multi-turn movement
---planning, and threat zone avoidance. Primary pathfinding for battlescape combat.
---
---Pathfinding Features:
---  - A* algorithm with hex grid heuristics
---  - Terrain-based movement costs (grass=1, rubble=2, water=3, etc.)
---  - Time Unit (TU/AP) cost calculation
---  - Multi-turn path planning
---  - Threat zone avoidance (enemy overwatch)
---  - Cover-aware pathfinding
---  - Binary heap priority queue optimization
---
---Movement Costs:
---  - Base terrain cost (from terrain_movement_costs.lua)
---  - Elevation changes (+1 TU per level)
---  - Threat zones (+2 TU penalty)
---  - Cover bonuses (path through cover preferred)
---
---Key Exports:
---  - Pathfinding.findPath(start, goal, unit, map): Calculates optimal path
---  - Pathfinding.getMovementCost(tile, unit): Returns TU cost for tile
---  - Pathfinding.isValidMove(tile, unit): Checks if move is legal
---  - Pathfinding.calculatePathCost(path, unit): Total TU cost for path
---  - Pathfinding.getReachableTiles(unit, maxTU): All tiles within TU range
---
---Dependencies:
---  - battlescape.maps.grid_map: Battlefield grid representation
---  - battlescape.combat.unit: Unit stats and movement properties
---  - assets.data.terrain_movement_costs: Terrain cost lookup table
---
...
---  local path = Pathfinding.findPath(startTile, goalTile, unit, battlefield.map)
---  local cost = Pathfinding.calculatePathCost(path, unit)
---  if cost <= unit.currentTU then
---    unit:moveAlongPath(path)
---  end
---
---@see battlescape.maps.grid_map For battlefield grid
---@see battlescape.combat.unit For unit movement
---@see ai.tactical.decision_system For AI movement
