...
---
--- This module manages rendering of the tactical map, including terrain,
--- units, fog of war, and UI overlays. Optimizes rendering by only drawing
--- visible tiles and supports debug visualization modes.
---
--- Example usage:
---   local renderer = BattlefieldRenderer.new(24)
---   renderer:draw(battlefield, camera, teamManager, currentTeam, false, 960, 720)

--- @class BattlefieldRenderer
--- @field tileSize number Size of each tile in pixels
--- @field showGrid boolean Whether to show grid overlay
--- @field debugViewTeam string|nil Team ID for debug visibility view
local BattlefieldRenderer = {}
BattlefieldRenderer.__index = BattlefieldRenderer

--- Create a new battlefield renderer.
---
--- @param tileSize number Size of each tile in pixels (default: 24)
--- @return table New BattlefieldRenderer instance
...
--- Set debug view team for visibility debugging.
---
--- When set, shows the battlefield from specified team's perspective.
---
--- @param teamId string|nil Team ID or nil for normal view
--- @return nil
...
---
--- Renders terrain, fog of war, and grid within camera viewport.
--- Only renders tiles that are visible on screen for performance.
---
...
--- @param currentTeam table Current active team
--- @param isNight boolean Whether it's night time (affects brightness)
--- @param viewportWidth number Screen viewport width
--- @param viewportHeight number Screen viewport height
--- @return nil
