---MoveModeSystem - Movement Mode Management (ECS)
---
---Manages different movement modes (WALK, RUN, SNEAK, FLY) with costs, benefits,
---and restrictions. Part of the ECS (Entity-Component-System) battle architecture.
---Determines AP costs and tactical effects of movement types.
---
---Features:
---  - Multiple movement modes (walk, run, sneak, fly)
---  - AP cost calculation per mode
---  - Armor integration (flight suits enable FLY)
---  - Speed modifiers
---  - Detection penalties (sneak reduces visibility)
---  - Mode restrictions and validation
---
---Movement Modes:
---  - WALK: Normal movement, 1 AP/tile
---    * Default mode, balanced speed/stealth
---    * No penalties or bonuses
---
---  - RUN: Fast movement, 0.5 AP/tile
---    * Double speed but increased visibility
---    * Higher detection chance
---    * Cannot shoot after running
---
---  - SNEAK: Slow movement, 2 AP/tile
---    * Half speed but reduced detection
---    * Lower visibility to enemies
---    * Better ambush positioning
---
---  - FLY: Flying movement, 1 AP/tile
---    * Requires flight-capable armor
---    * Ignores ground obstacles
---    * Can reach elevated positions
---
---Mode Switching:
---  - Free action (0 AP)
---  - Can switch before movement
---  - Mode persists between turns
---  - Armor restrictions apply
---
---Key Exports:
---  - MODES: Table of mode definitions
---  - setMoveMode(unit, modeId): Changes unit's mode
---  - getMoveMode(unit): Returns current mode
---  - getMoveCost(unit, distance, modeId): Returns AP cost
---  - canUseMode(unit, modeId): Returns true if mode available
---
---Dependencies:
---  - None (standalone system)
---
---@module battlescape.battle_ecs.move_mode_system
...
---  MoveModeSystem.setMoveMode(unit, "run")
---  local cost = MoveModeSystem.getMoveCost(unit, 5, "run")
---
---@see battlescape.battle_ecs.movement_system For movement execution
