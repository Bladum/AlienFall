...
--- Assert that two values are not equal (legacy compatibility)
-- @param actual any: The actual value
-- @param unexpected any: The value it should not equal
-- @param message string: Optional custom message
function test_framework.assert_not_equal(actual, unexpected, message)
    if actual == unexpected then
        local msg = message or string.format("Expected %s to not equal %s", tostring(actual), tostring(unexpected))
        error(msg, 2)
    end
end
