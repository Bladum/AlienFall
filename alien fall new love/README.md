...
For development guidelines, project structure, and contribution information, see [DEVELOPMENT.md](DEVELOPMENT.md).

## Testing

The project includes a comprehensive test suite designed to work like pytest for Python, providing easy test discovery, execution, and reporting.

### Running Tests (pytest-style)

- **Run All Tests**: `Ctrl+Shift+T` in VS Code or `love . --test`
- **Run Specific Test**: `Ctrl+Shift+R` in VS Code or `love . --test-file test.test_toml_loader`
- **Watch Mode**: `love . --test-watch` for continuous testing
- **From Command Line**: `./run_game.ps1 -Test` or `./run_game.bat --test`

### Test Features

- **Automatic Discovery**: Scans `test/` directory for `test_*.lua` files
- **Selective Execution**: Run individual test files or all tests
- **Headless Mode**: Tests run without opening a game window
- **Detailed Output**: Clear pass/fail reporting with failure details
- **Exit Codes**: Proper success/failure codes for CI/CD integration

### Writing Tests

Tests follow a simple structure with assertions and automatic discovery. See [test/README.md](test/README.md) for complete documentation and examples.
