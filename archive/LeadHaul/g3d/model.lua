...
local function newModel(verts, texture, translation, rotation, scale)
	...
	-- fill color, an alpha of 0 will render the model normally
...
function model:draw(shader)
    ...
    shader:send("fillColor", self.fillColor )
...
-- quick and dirty clone function
function model:clone()
	local clone = g3d.newModel(
	self.verts,
	self.texture,
	{ self.translation[1],self.translation[2],self.translation[3] },
	{ self.rotation[1],self.rotation[2],self.rotation[3] },
	{ self.scale[1],self.scale[2],self.scale[3] }
	)
