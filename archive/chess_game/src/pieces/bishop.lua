-- Bishop piece - moves on diagonals, max range 3
local Constants = require("src.core.constants")
local Piece = require("src.pieces.piece")

local Bishop = setmetatable({}, {__index = Piece})
Bishop.__index = Rook

function Bishop.new(x, y, playerId)
    local self = setmetatable(Piece.new(x, y, playerId, Constants.PIECE_TYPES.BISHOP), Bishop)
    return self
end

function Bishop:canMoveTo(board, toX, toY)
    ...
    -- Must be on diagonal
    if not self:isDiagonal(self.x, self.y, toX, toY) then
        return false
    end
...
function Bishop:getValidMoves(board)
    local moves = {}
