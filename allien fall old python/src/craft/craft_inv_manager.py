...
from enums import ECraftItemCategory
...
class CraftInventoryTemplate:
    ...
    Attributes:
        ...
        hardpoint_data (Dict[str, Optional[Dict[str, object]]]): Mapping of hardpoint names to item data dicts.
        cargo_data (Dict[str, object]): Mapping of cargo item names to item data dicts.
    """
    def __init__(self, name: str, hardpoint_data: Dict[str, Optional[Dict[str, object]]], cargo_data: Dict[str, object]) -> None:
        """
    ...
    def to_dict(self) -> Dict[str, object]:
        ...
        Returns:
            Dict[str, object]: Serialized template data.
        """
        return {
            'name': self.name,
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, object]) -> 'CraftInventoryTemplate':
        """
        Deserialize a CraftInventoryTemplate from a dictionary.

        Args:
            data (Dict[str, object]): Serialized template data.
...
class TCraftInventoryManager:
    ...
    Attributes:
        ...
        _template (Optional[Dict[str, object]]): Cached template data for the current loadout.
    ...
    def can_accept_component(self, component: TItem, hardpoint_name: str) -> bool:
        ...
        if hardpoint_name.startswith("Weapon"):
            component_type = component.item_type
            component_type_str = str(component_type).lower()
    ...
    def mount_component(self, hardpoint_name: str, component: TItem) -> bool:
        ...
        # ENFORCE: Remove hasattr, trust type system
    ...
    def load_template(self, template: Dict[str, object]):
        """
        Load an inventory configuration from a template (dict).

        Args:
            template (Dict[str, object]): Serialized template data.
        """
    ...
    def to_dict(self) -> Dict[str, object]:
        ...
        Returns:
            Dict[str, object]: Serialized inventory data.
        """
        result = {
            'hardpoints': {},
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, object], item_factory) -> 'TCraftInventoryManager':
        """
        Deserialize a TCraftInventoryManager from a dictionary.

        Args:
            data (Dict[str, object]): Serialized inventory data.
    ...
    def _apply_performance_modifiers(self, hardpoint_name: str, modifiers: Dict[str, int]) -> None:
        """
        ...
        for stat_name, modifier in modifiers.items():
            # Direct, type-safe stat access only
            if stat_name == "speed":
                self.craft.speed += modifier
            elif stat_name == "armor":
                self.craft.armor += modifier
            # ...add all other known stats here...
            else:
                raise AttributeError(f"Unknown stat: {stat_name}")

    def _remove_performance_modifiers(self, hardpoint_name: str, modifiers: Dict[str, int]) -> None:
        """
