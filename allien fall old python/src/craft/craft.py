...
if TYPE_CHECKING:
    from missions.mission import TCraftMission
...
class TCraft(TLocation):
    ...
    def __init__(self, pid: str, data: dict[str, object] = {}):
        ...
        position: List[float] = data.get('position', [0, 0])
        units: List[str] = data.get('units', [])
        pilots: List[str] = data.get('pilots', [])
        items: Dict[str, int] = data.get('items', {})
        ...
        self.crew: List[TUnit] = data.get('crew', [])  # List of crew/unit objects
        self.mission: Optional['TCraftMission'] = None  # Mission assignment
    ...
    def get_crew(self) -> List[TUnit]:
        """
        Get the list of crew members.

        Returns:
            List[TUnit]: List of crew/unit objects.
        """
        return self.crew
    ...
    # Mission assignment
    def assign_mission(self, mission: 'TCraftMission') -> None:
        """
        Assign a mission to the craft.

        Args:
            mission (TCraftMission): Mission assignment data.
        """
        self.mission = mission
        # Use direct attribute access, trusting the type system
        self.add_notification(f"Mission assigned: {mission.name}")
    ...
    def complete_mission(self) -> None:
        ...
        """
        if self.mission:
            self.add_notification(f"Mission completed: {self.mission.name}")
            self.mission = None

    def get_mission_status(self) -> Optional['TCraftMission']:
        """
        Get the current mission assignment.

        Returns:
            Optional[TCraftMission]: Current mission data, or None if not assigned.
        """
        return self.mission
