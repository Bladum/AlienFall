...
class BlackMarketSupplier:
    ...
    def __init__(self, name: str, data: Optional[Dict[str, object]] = None):
        """
        ...
        self.name: str = name
        self.reputation: str = data.get('reputation', 'unknown')
        self.region_specialization: List[str] = data.get('region_specialization', [])
        self.item_specialization: List[str] = data.get('item_specialization', [])
        self.availability_schedule: str = data.get('availability_schedule', 'weekly')
        self.price_variance_range: Tuple[float, float] = data.get('price_variance_range', (0.8, 1.5))
        self.reliability: float = data.get('reliability', 0.8)
        self.discovery_requirements: Dict[str, object] = data.get('discovery_requirements', {})
        self.is_discovered: bool = data.get('is_discovered', False)
        self.last_refresh: datetime = data.get('last_refresh', datetime.now())
        self.current_stock: List[TPurchaseEntry] = data.get('current_stock', [])
        
        # Stock configuration: can be extended for custom stock logic per supplier
        self.stock_size_range: Tuple[int, int] = tuple(data.get('stock_size_range', [3, 8]))
        self.rare_item_chance: float = data.get('rare_item_chance', 0.3)
    ...
    def refresh_stock(self, available_entries: List[TPurchaseEntry]):
        """
        ...
        eligible_entries: List[TPurchaseEntry] = []
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, object]) -> 'BlackMarketSupplier':
        """
        ...
        # current_stock should be rebuilt as List[TPurchaseEntry] elsewhere
        return supplier
...
class BlackMarket:
    ...
    def __init__(self):
        ...
        self.discovery_events: List[dict] = []
        self.global_reputation: float = 0.0  # Affects prices and availability
    ...
    def refresh_all_stock(self, available_entries: List[TPurchaseEntry]):
        """
    ...
    def get_supplier_stock(self, supplier_name: str) -> List[TPurchaseEntry]:
        """
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, object]) -> 'BlackMarket':
        """
