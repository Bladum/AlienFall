...
from economy.purchase_manager import PurchaseManager
from economy.black_market import BlackMarket
...
class TPurchase:
    ...
    Attributes:
        entries (Dict[str, TPurchaseEntry]): entry_id -> TPurchaseEntry
        purchase_manager (PurchaseManager): Manages active orders
        black_market (BlackMarket): Manages black market suppliers
    """

    def __init__(self, purchase_data: Optional[Dict[str, dict]] = None):
        """
        Initialize purchase system.

        Args:
            purchase_data (Dict[str, dict], optional): Purchase configuration data
        """
        self.entries: Dict[str, TPurchaseEntry] = {}
        self.purchase_manager: PurchaseManager = PurchaseManager()
        self.black_market: BlackMarket = BlackMarket()
    ...
    def _load_purchase_data(self, data: Dict[str, dict]):
        """
        Load purchase entries and black market data from configuration.

        Args:
            data (Dict[str, dict]): Purchase configuration data
        """
