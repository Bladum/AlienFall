...
from engine.mod import TMod, TUnitCategory, TItemCategory
...
class TGame:
    ...
    def get_current_base_units(self) -> List[TUnit]:
        ...
        Returns:
            List[TUnit]: List of TUnit objects for the active base
        """
        active_base = self.get_active_base()
        if not active_base:
            return []
        return active_base.inventory.units

    def get_current_base_items(self) -> List[Tuple[str, float, Optional[EUnitItemCategory]]]:
        """
        Get items for the currently active base.

        Returns:
            List[Tuple[str, float, Optional[EUnitItemCategory]]]: Items from the base inventory
        """
        active_base = self.get_active_base()
        if not active_base:
            return []
        return active_base.inventory.get_all_items()
    ...
    def get_current_base_crafts(self) -> List[TCraft]:
        ...
        Returns:
            List[TCraft]: List of TCraft objects for the active base
        """
        active_base = self.get_active_base()
        if not active_base:
            return []
        # Crafts are always in inventory.crafts (enforced by type)
        return active_base.inventory.crafts
    ...
    def get_base_summary(self) -> Dict[str, int]:
        ...
        Returns:
            Dict[str, int]: Dictionary containing unit counts by category and base capacity
        """
        active_base = self.get_active_base()
        if not active_base:
        ...
        for unit in active_base.inventory.units:
            # Determine category from unit_type or fallback
            category = unit.unit_type.category
            if category is None:
                # Fallback: use unit_type.name or id to guess category
                unit_type_name = unit.unit_type.name.lower()
                if 'tank' in unit_type_name:
                    category = 'tank'
                elif 'dog' in unit_type_name or 'pet' in unit_type_name:
                    category = 'dog'
                elif 'alien' in unit_type_name:
                    category = 'alien'
                else:
                    category = 'soldier'
            if category == 'soldier':
        ...
        # Use unit space as personnel capacity (assume method exists)
        capacity = active_base.get_unit_space()
    ...
    def initialize_starting_bases(self) -> bool:
        ...
        Returns:
            bool: True if bases were successfully initialized, False otherwise
        """
        ...
        if not self.mod or not self.mod.starting_base:
            ...
            for base_name, base_data in self.mod.starting_base.items():
                ...
                for craft_data in base_data.get('crafts', []):
                    ...
                    if craft_type_id and craft_type_id in self.mod.craft_types:
                        ...
                        if 'name' in craft_data:
                            new_craft.name = craft_data['name']
                        # Always assume new_base.crafts exists and is a list
    ...
    def get_unit_by_name(self, unit_name: str) -> Optional[TUnit]:
        """Return the TUnit object for the given name from the current base, or None if not found."""
        units = self.get_current_base_units()
        for unit in units:
