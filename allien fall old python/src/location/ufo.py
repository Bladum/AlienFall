...
class TUfo(TLocation):
    ...
    """
    def __init__(self, ufo_id: str, data: dict[str, object]):
        """
        ...
        Attributes:
            ufo_id (str): UFO ID.
            position (tuple[int, int] | None): Tile or coordinates on world map.
            ufo_type (TUfoType | None): Reference to TUfoType instance.
            ufo_script (TUfoScript | None): Reference to TUfoScript instance.
            script_step (int): Current script step.
        ...
        self.ufo_id: str = ufo_id
        self.position: tuple[int, int] | None = data.get('position')
        ufo_type = data.get('ufo_type')
        ufo_script = data.get('ufo_script')
        self.ufo_type = self.game.mod.ufo_types.get(ufo_type)
        self.ufo_script = self.game.mod.ufo_scripts.get(ufo_script)
        self.script_step: int = 0
        self.speed: int = 0
        self.speed_max: int = self.ufo_type.speed if self.ufo_type else 0
        self.health: int = self.ufo_type.health if self.ufo_type else 0

    def advance_script(self, turns: int = 1):
        """
    ...
    def get_position(self):
        ...
        """
        # Assume position is always set and valid
    ...
    def distance_to(self, other):
        ...
        """
        if other.get_position is not None:
