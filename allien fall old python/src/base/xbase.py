...
class TBaseXCom(TLocation):
    ...
    Attributes:
        facilities (dict[tuple, TFacility]): Mapping of (x, y) positions to TFacility objects.
        inventory (TBaseInventory): Inventory system for items, units, crafts, and captures.
        game (TGame): Reference to the game object for mod and research checks.
    """
    def __init__(self, pid, data: dict = {}):
        ...
        self.inventory: TBaseInventory = TBaseInventory(storage_capacity=storage_capacity, craft_capacity=craft_capacity)
    ...
    def get_storage_space(self) -> float:
        return sum(f.facility_type.storage_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_unit_space(self) -> float:
        return sum(f.facility_type.unit_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_prison_space(self) -> float:
        return sum(f.facility_type.prison_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_alien_space(self) -> float:
        return sum(f.facility_type.alien_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_craft_space(self) -> float:
        return sum(f.facility_type.craft_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_hospital_space(self) -> float:
        return sum(f.facility_type.hospital_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_training_space(self) -> float:
        return sum(f.facility_type.training_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_workshop_space(self) -> float:
        return sum(f.facility_type.workshop_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_research_space(self) -> float:
        return sum(f.facility_type.research_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_psi_space(self) -> float:
        return sum(f.facility_type.psi_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_relax_space(self) -> float:
        return sum(f.facility_type.relax_space for f in self.get_active_facilities() if f.facility_type is not None)

    def get_repair_space(self) -> float:
        return sum(f.facility_type.repair_space for f in self.get_active_facilities() if f.facility_type is not None)
    ...
    def get_radar_facilities(self):
        ...
        for facility in self.facilities.values():
            ...
            # Direct attribute access, assume all radar attributes exist
            radar_range = ftype.radar_range
            radar_power = ftype.radar_power
    ...
    def save_data(self) -> dict:
        ...
        # Direct method call, assume inventory always has save_data
    ...
    def load_data(self, data: dict) -> None:
        ...
        for facility_data in facilities_data:
            ...
            if pos and facility_type_id:
                from engine.game import TGame
                game = TGame()
                facility_type = game.mod.facilities[facility_type_id]
