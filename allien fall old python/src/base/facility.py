...
class TFacility:
    ...
    """
    def __init__(self, facility_name: str, position: tuple[int, int] = None) -> None:
        """
        Initialize a facility instance in a base.

        Args:
            facility_name (str): Facility type ID or name.
            position (tuple[int, int]): (x, y) grid position in base.
        """
        ...
        self.position: tuple[int, int] = position  # (x, y) or similar
        self.build_progress: int = 0  # days built so far
        self.completed: bool = False
        self.hp: int = 10  # default, can be extended
