...
class TBaseAlien(TLocation):
    ...
    def __init__(self, pid: str, data: dict[str, object] = None) -> None:
        ...
        Args:
            pid (str): Unique base identifier.
            data (dict[str, object]): Dictionary with base properties (location, name, etc).
        """
        ...
        self.level: int = 1  # Always starts at level 1
        self.level_max: int = 4
        self.level_up_pending: bool = False
        self.month_progress: int = 0  # Days since last mission cycle
        self.missions_this_month: int = 0
        self.supply_pending: bool = False
        self.score_per_day: int = 5  # Per level
        self.units_per_level: dict[int, int] = {1: 20, 2: 30, 3: 40, 4: 50}
        self.map_size_per_level: dict[int, int] = {1: 4, 2: 5, 3: 6, 4: 7}
        self.max_missions_per_month: dict[int, int] = {1: 1, 2: 2, 3: 3, 4: 4}
        self.days_in_month: int = 30
        self.missions_types: list[str] = [
            'research', 'supply', 'hunt', 'infiltration', 'retaliation', 'create_base'
        ]
        self.region_id: Any = self.region_id
        self.pending_missions: list[str] = []  # Missions to be launched this month
