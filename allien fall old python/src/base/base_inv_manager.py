...
class TBaseInventory:
    ...
    def __init__(self, storage_capacity: int = 100, craft_capacity: int = 4) -> None:
        """
    ...
    def get_all_items(self) -> List[Tuple[str, float, Optional[EUnitItemCategory]]]:
        """
        Return a list of all items in the inventory with their quantities and categories.

        Returns:
            List[Tuple[str, float, Optional[EUnitItemCategory]]]: (item_id, quantity, category)
        """
        return [
            (item_id, quantity, self.item_categories.get(item_id))
    ...
    def save_template(self) -> Dict[str, object]:
        ...
        Returns:
            Dict[str, object]: Dictionary representing the current items, units, crafts, and captures.
        """
        return {
    ...
    def load_template(
        ...
        unit_provider: Optional[callable[[str], TUnit]] = None,
        craft_provider: Optional[callable[[str], TCraft]] = None
    ) -> None:
        """
        Load base inventory from a template dict.

        Args:
            template (Dict[str, object]): Dictionary representing items, units, crafts, and captures.
            unit_provider (Optional[Callable[[str], TUnit]]): Function to get TUnit by id.
            craft_provider (Optional[Callable[[str], TCraft]]): Function to get TCraft by id.
        """
        self.items = dict(template.get('items', {}))
        self.captures = dict(template.get('captures', {}))
        if unit_provider is not None:
            self.units = [unit_provider(uid) for uid in template.get('units', [])]
        if craft_provider is not None:
