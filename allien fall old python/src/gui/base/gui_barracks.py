...
class TGuiBarracks(TGuiCoreScreen):
    ...
    def _setup_unit_list(self) -> None:
        ...
        # Load initial units
        for unit in self.game.get_current_base_units():
            name = unit.name
            unit_type = unit.unit_type
            unit_icon = self.game.mod.tileset_manager.get_tile(unit_type)
            item = QListWidgetItem(QIcon(unit_icon), unit.name)
    ...
    def _setup_item_list(self) -> None:
        ...
        for name, item_data in self.game.get_current_base_items():
            ...
            count = item_data.quantity  # Assume quantity always present
    ...
    def _update_equipment_slot_states(self, enabled_count: int):
        ...
        equipment_slots = [slot for slot in self.equipment_slots if slot.slot_type == EUnitItemCategory.EQUIPMENT]
    ...
    def _on_unit_selected(self, unit_name: str):
        ...
        if unit:
            ...
            exp_level = unit.level
            # Use direct attribute access for classes
            try:
                class_list = unit.classes
            except AttributeError:
                try:
                    class_list = unit.unit_type.classes
                except AttributeError:
                    class_list = []
            class_str = ', '.join(str(cls) for cls in class_list) if class_list else 'None'

            unit_info_text =  f"Unit:  {unit_name}\n"
            unit_info_text += f"Type:  {unit_type_name}\n"
            unit_info_text += f"Classes: {class_str}\n"
            unit_info_text += f"Level: {exp_level}\n"
    ...
    def _calculate_combat_effectiveness(self, unit: TUnit) -> float:
        ...
        for stat in base_stats:
            if stat == "health":
                base_values.append(unit.health)
            elif stat == "stamina":
                base_values.append(unit.stamina)
            elif stat == "strength":
                base_values.append(unit.strength)
            elif stat == "reactions":
                base_values.append(unit.reactions)
            elif stat == "accuracy":
                base_values.append(unit.accuracy)
        if not base_values:
            return 0.0
        ...
        equipment_bonus = 0.0
        # Assume unit.inventory always exists
        has_weapon = any(item.item_type == TItemType.ITEM_WEAPON for item in unit.inventory)
        has_armor = any(item.item_type == TItemType.ITEM_ARMOR for item in unit.inventory)
        ...
        if template and template.equipment_data:
            for slot in self.equipment_slots:
                ...
                if slot_key in template.equipment_data and template.equipment_data[slot_key]:
                    try:
                        item = TItem.from_dict(template.equipment_data[slot_key], self.game.mod.item_types)
                        slot.add_item(item)
            ...
            if "Default" in self.saved_templates:
                ...
                for slot in self.equipment_slots:
                    ...
                    if slot_key in default_template.equipment_data and default_template.equipment_data[slot_key]:
                        try:
                            item = TItem.from_dict(default_template.equipment_data[slot_key], self.game.mod.item_types)
                            slot.add_item(item)
                        except Exception as e:
    ...
    def _update_unit_stats(self, unit_name: str):
        ...
        """
        unit : TUnit = self.game.get_unit_by_name(unit_name)
        if not unit or not self.stat_bars:
            return

        # Update each stat progress bar with the unit's current stat value from unit.stats
        for stat, bar in self.stat_bars.items():
            if stat == "health":
                value = unit.stats.health
            elif stat == "stamina":
                value = unit.stats.stamina
            elif stat == "strength":
                value = unit.stats.strength
            elif stat == "reactions":
                value = unit.stats.reactions
            elif stat == "accuracy":
                value = unit.stats.accuracy
            elif stat == "throwing":
                value = unit.stats.throwing
            else:
                value = 0
            bar.setValue(value)
    ...
    def _update_unit_equipment_status(self, unit_name: str):
        ...
        for slot in self.equipment_slots:
            if slot.slot_type == EUnitItemCategory.EQUIPMENT:
