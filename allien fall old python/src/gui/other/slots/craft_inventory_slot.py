...
class TCraftInventorySlot(TInventorySlot):
    ...
    def __init__(
        ...
        self.compatibility: Dict[str, object] = {}  # Compatibility requirements
    ...
    def can_accept_item(self, item) -> bool:
        ...
        if self.craft:
            ...
            if self.craft.craft_type and 'craft_type' in compatibility and self.craft.craft_type not in compatibility['craft_type']:
                return False

            # Check size compatibility
            if self.craft.size and 'min_size' in compatibility and self.craft.size < compatibility['min_size']:
                return False

            # Check hardpoint compatibility
            if self.hardpoint_type and 'hardpoint' in compatibility and self.hardpoint_type not in compatibility['hardpoint']:
                return False
    ...
    def add_item(self, item: TItem, quantity: int = 1, emit_signal: bool = True) -> bool:
        ...
        if self.craft:
            ...
            for stat, value in stat_modifiers.items():
                ...
                if stat == "speed":
                    self.craft.speed += value
                elif stat == "armor":
                    self.craft.armor += value
                # ...add all other known stats here...
                else:
                    raise AttributeError(f"Unknown stat: {stat}")
            self.stat_modifiers = stat_modifiers

            # Add component to craft's internal systems (assume method exists)
    ...
    def set_compatibility(self, compatibility: Dict[str, object]):
        """
