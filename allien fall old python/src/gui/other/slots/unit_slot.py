...
class TUnitSlot(TInventorySlot):
    ...
    def update_tooltip(self):
        ...
        rank = self.unit.rank
        if rank:
            tooltip += f"Rank: {rank}<br>"

        skills = self.unit.skills
        if isinstance(self.unit.skills, dict):
            relevant_skills = []
    ...
    def can_accept_unit(self, unit) -> bool:
        ...
        if self.position_type == "pilot":
            skills = unit.skills
            if skills.get('piloting', 0) < self.requirements.get('piloting', 0):
                return False

        elif self.position_type == "navigator":
            skills = unit.skills
            if skills.get('navigation', 0) < self.requirements.get('navigation', 0):
                return False
    ...
    def create_mime_data(self):
        ...
        mime_data = super().create_mime_data()
        if self.unit:
            unit_data = {"unit_id": self.unit.id}
    ...
    def create_drag_pixmap(self):
        ...
        if not self.unit or not callable(self.unit.get_sprite):
            return None
    ...
    def handle_right_click(self):
        ...
        # Example: Show more detailed tooltip
        if self.unit:
    ...
    def get_detailed_unit_info(self) -> str:
        ...
        experience = self.unit.experience
        if experience:
            details += f"<b>Experience:</b> {experience}<br>"
        missions = self.unit.missions
        if missions:
            details += f"<b>Missions:</b> {missions}<br>"
