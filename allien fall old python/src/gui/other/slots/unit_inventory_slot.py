...
class TUnitInventorySlot(TInventorySlot):
    ...
    def update_unit_restrictions(self):
        ...
        unit_strength = self.unit.strength
        if (required_strength > 0 and unit_strength < required_strength):
    ...
    def can_accept_item(self, item) -> bool:
        ...
        # Assume item.requirements and self.unit.strength always exist
    ...
    def add_item(self, item: TItem, quantity: int = 1, emit_signal: bool = True) -> bool:
        ...
        for stat, value in item.stat_modifiers.items():
            if stat == "health":
                self.unit.health += value
            elif stat == "strength":
                self.unit.strength += value
            elif stat == "stamina":
                self.unit.stamina += value
            elif stat == "reactions":
                self.unit.reactions += value
            elif stat == "accuracy":
                self.unit.accuracy += value
            elif stat == "psi":
                self.unit.psi += value
            elif stat == "bravery":
                self.unit.bravery += value
            # Add more stat names as needed
        self.stat_modifiers = item.stat_modifiers
