...
class TCampaign:
    ...
    def __init__(self, pid: str, data: dict[str, Any]) -> None:
        ...
        Args:
            pid (str): Campaign identifier.
            data (dict[str, Any]): Campaign data (name, score, objective, faction, tech_start, tech_end, regions, missions).
        """
        self.pid: str = pid
        self.name: str = data.get('name', '')
        self.score: int = data.get('score', 0)
        self.objective: int = data.get('objective', 0)
        self.faction: str = data.get('faction', '')
        self.tech_start: list[str] = data.get('tech_start', [])
        self.tech_end: list[str] = data.get('tech_end', [])
        self.regions: dict[str, int] = {}
        if 'regions' in data and isinstance(data['regions'], dict):
            self.regions = data['regions']
        self.missions: list[TMission] = []
